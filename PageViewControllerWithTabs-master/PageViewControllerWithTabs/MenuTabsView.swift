   ...
   lazy var collView: UICollectionView = {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if isSizeToFitCellsNeeded {
            ...
            //let estimatedRect = NSString.init(string: str).boundingRect(with: sizeee, options: options, attributes: [NSAttributedStringKey.font : UIFont.boldSystemFont(ofSize: 40)], context: nil)
        
            return CGSize.init(width: (self.frame.width - 280), height: self.frame.height)
